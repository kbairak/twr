...
class EventGenerator:
    def __init__(
        ...
        # Will store UUIDs after initialization
    ...
    async def connect(self):
        ...
        await self._initialize_entities()

    async def _initialize_entities(self):
        """Create users and products in DB"""
        for user in self.users:
            user_id = await self.conn.fetchval(
                'INSERT INTO "user" (name) VALUES ($1) RETURNING id', user
            )
            self.user_ids[user] = user_id
        for product in self.products:
            product_id = await self.conn.fetchval(
                "INSERT INTO product (name) VALUES ($1) RETURNING id", product
            )
    ...
    async def generate_and_insert(
        ...
        for i, tick_time in enumerate(generate_trading_timestamps(
            ...
            for product in self.products:
                ...
                price_events.append(
                    PriceUpdate(
                        product_id=self.product_ids[product],
                        timestamp=timestamp,
                        price=price,
                    )
                )
        ...
        start_time = min(e.timestamp for e in price_events)
        end_time = max(e.timestamp for e in price_events)
    ...
    def _generate_cashflow_event(self, timestamp: datetime):
        ...
        # Return Cashflow dataclass instance
        return Cashflow(
            ...
            execution_money=execution_money,
            user_money=execution_money,
    ...
    async def _batch_insert_all_events(
        ...
        # Insert price events
        if price_events:
            price_events.sort(key=lambda x: (x.product_id, x.timestamp))
...
The third will be calculated automatically.
    ...
    print("\nDone! Events have been inserted.")
